...
const useCanvasEventHandlers = (
    ...
    useEffect(() => {
      ...
      const handlePathCreated = (e: any) => {
        if (!e.path || !canvas.contains(e.path)) {return;}
      ...
      const handleEraserAdded = (e: any) => {
        ...
        e.targets.forEach((target: fabric.Object) => {
          ...
          if (!clonedJSON.eraser.objects) {delete clonedJSON.eraser;}
      ...
      const handleTextEditingExited = (e: any) => {
        if (e.target._id !== state.observedTextId || e.target.text === state.observedTextContent) {return;}
    ...
    }, [canvas, saveCommand, handleStyleChange, state.observedTextContent, state.observedTextId, state.recentlyActiveObjects]);
  };
